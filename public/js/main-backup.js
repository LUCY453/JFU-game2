...
class GameRenderer {
    ...
    addRoads() {
        // 主干道 - 横向
        const mainRoadH = new THREE.PlaneGeometry(100, 4);
        ...
        // 主干道 - 纵向
        const mainRoadV = new THREE.PlaneGeometry(4, 100);
        ...
        // 横向人行道
        for (let i = -1; i <= 1; i += 2) {
            const sidewalk = new THREE.PlaneGeometry(100, 1);
        ...
        // 纵向人行道
        for (let i = -1; i <= 1; i += 2) {
            const sidewalk = new THREE.PlaneGeometry(1, 100);
            const walk = new THREE.Mesh(sidewalk, sidewalkMaterial);
            walk.rotation.x = -Math.PI / 2;
    ...
    createBuilding(width, height, depth, color, x, y, z) {
        ...
        for (let i = -2; i <= 2; i++) {
            if (i !== 0) { // 避开门的位置
                ...
                window.position.set(i * 3, height/4, depth/2 + 0.05);
                group.add(window);
            }
        }
    ...
    checkCollision(position) {
        ...
        for (let building of this.buildings) {
            ...
            if (buildingBox.intersectsBox(playerBox)) {
                ...
                const doorZ = buildingPos.z + (building.userData?.depth || 15) / 2;
                const doorMinX = buildingPos.x - 1;
                const doorMaxX = buildingPos.x + 1;
                
                // 如果在门的范围内，允许进入
                if (position.x >= doorMinX && position.x <= doorMaxX && 
                    Math.abs(position.z - doorZ) < 1) {
                    continue; // 不阻挡，可以进入
                }
                
                return true; // 发生碰撞，阻挡移动
            }
        }
